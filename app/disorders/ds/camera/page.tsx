...
const useCamera = () => {
  ...
  const initializeCamera = async () => {
    try {
      ...
      <div className="relative aspect-square bg-black rounded-3xl overflow-hidden shadow-xl"></div>;
    }
  };
...
export default function AssessmentTestPage() {
  ...
  useEffect(() => {
    if (videoState.url && videoRef.current?.srcObject) {
      const tracks = (videoRef.current.srcObject as MediaStream).getTracks();
      tracks.forEach((track) => track.stop());
      videoRef.current.srcObject = null;
      // Stop video element display
      if (videoRef.current) {
        ...
        >
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div
                className={`relative aspect-square bg-black rounded-3xl overflow-hidden shadow-xl ${
                  videoState.url ? "hidden" : ""
                }`}
              >
                <video
                  ref={videoRef}
                ...
                <>
                  <div className="space-y-6">
                    <div className="space-y-4 text-sm">
                      <h2 className="text-md font-semibold">Guidelines</h2>
